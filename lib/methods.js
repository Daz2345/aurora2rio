Meteor.methods({
    'Activities.insert.manual' (activity) {
        var activitySubmit = {
            ...
            userId: this.userId,
            username: (Meteor.user().profile !== undefined) ? Meteor.user().profile.fullName.replace("-", " ") : "No name submitted",
            teamName: (Meteor.user().team !== "" && Meteor.user().team !== undefined) ? Teams.findOne({_id: Meteor.user().team}).name : "No Team",
